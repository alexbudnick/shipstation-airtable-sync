...
export async function upsertShipStationProductBySku(existingProduct, payload) {
  if (CFG.dryRun) {
    logger("info", existingProduct ? "DRY_RUN would update ShipStation product" : "DRY_RUN would create ShipStation product", payload);
    return { dryRun: true };
  }
  ...
  return shipstationRequest(`/products`, {
    method: "POST",
